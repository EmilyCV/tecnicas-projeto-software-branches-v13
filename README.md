...
> ⚠️ A seção [Solução de Problemas](#solução-de-problemas) contêm soluções para problemas comuns. Não deixam de conferir caso encontre dificuldades.
...
│   │       ├── ...
│   │       └── db/
│   │           └── changelog/        # Migrações do Liquibase
│   │               ├── core/         # Migrações e dados principais
│   │               └── hotelaria/    # Migrações específicas de hotelaria
│   │               └── veiculo/      # Migrações específicas de locação de veiculos
│   └── test/
...
├── common                # Componentes e utilitários compartilhados (DTOs, exceptions, utils)
│   ├── config            # Configurações globais (OpenAPI, segurança, Stripe, web)
│   └── hibernate         # Geração e utilitários relacionados ao Hibernate (UUID generator, etc)
...
├── hotelaria             # Funcionalidades específicas de hotelaria
│   ├── domain            # Entidades de negócio e modelos de dados
│   ├── repository        # Interfaces de acesso a dados (Spring Data JPA)
│   ├── service           # Serviços e lógica de negócio
├── pagamento             # Funcionalidades específicas de pagamento
│   ├── domain            # Entidades de negócio e modelos de dados
│   ├── service           # Serviços e lógica de negócio
├── aereo                 # Funcionalidades específicas de aéreo
│   ├── domain            # Entidades de negócio e modelos de dados
│   ├── service           # Serviços e lógica de negócio
├── veiculo               # Modulo de locação de veiculos
│   └── internal          # Classes internas do módulo de locacao de veiculos
│       ├── domain        # Entidades de negócio e modelos de dados
│       ├── repository    # Interfaces de acesso a dados (Spring Data JPA)
│       └── service       # Serviços e lógica de negócio
```
...
O projeto utiliza o JaCoCo (Java Code Coverage) para medir a cobertura de código durante a execução dos testes automatizados. A cobertura de código indica a porcentagem do código-fonte que é executada pelos testes, ajudando a identificar áreas não testadas e melhorar a qualidade do software.

### Gerando Relatórios de Cobertura

Para gerar o relatório de cobertura de código, execute os testes com Maven:

```bash
./mvnw test
```

Ou, se quiser gerar apenas o relatório sem executar os testes novamente (caso já tenham sido executados):
...
Após a execução, o relatório HTML de cobertura estará disponível em:

- `target/site/jacoco/index.html`

Abra este arquivo em um navegador para visualizar o relatório interativo, que mostra a cobertura por classes, métodos e linhas de código.

Para mais detalhes sobre o JaCoCo, consulte a [documentação oficial](https://www.jacoco.org/jacoco/).
...
Essa sessão contVém soluções para alguns problemas comuns que podem ocorrer durante o desenvolvimento ou execução do projeto.

### Erro de compilação em classes geradas pelo Lombok

Classes que utilizam anotações do Lombok (como `@Data`, `@Builder`, etc) podem apresentar erros de compilação, de forma semelhante à Figura abaixo.

![image](docs/images/error-lombok.png)

Nesse caso, siga os passos abaixo.

####  Verifique que o plugin do lombok está habilitado.

Vá em `File` → `Settings` → `Plugins`, busque por "Lombok" e verifique se está instalado e habilitado. Se não estiver, instale e reinicie a IDE.

#### Verifique se o processamento de anotações está habilitado.

Vá em `File` → `Settings` → `Build, Execution, Deployment` → `Compiler` → `Annotation Processors` e verifique se a opção `Enable annotation processing` e `Obtain processors from project classpath` estão marcadas. 

Se não estiverem, marque-as e aplique as mudanças.


#### Verifique se a versão do JDK está correta.

Vá em `File` → `Project Structure` → `Platform Settings` → `SDK` e verifique se SDK está definido para Java 21. Se não estiver, selecione o JDK correto.

> **Dica:** O desenvolvimento deste projeto foi feito utilizando a [versão 21 do openjdk](https://openjdk.org/projects/jdk/21/).

### Erro de compilação em classes geradas pelo OpenAPI/Swagger generator

O processo de geração de código a partir do OpenAPI/Swagger pode **falhar** caso o **caminho** do código-fonte do projeto **contenha caracteres especiais** (como espaços, acentos, etc). A Figura abaixo exemplifica a saída de erro neste cenário.

![image](docs/images/error-path.png)

Nesse caso, mova o projeto para um diretório cujo caminho não contenha caracteres especiais e tente compilar novamente.
